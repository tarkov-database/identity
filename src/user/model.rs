...
impl Collection<UserDocument> {
    pub async fn get_by_id(&self, id: ObjectId) -> Result<UserDocument> {
    ...
    pub async fn get_by_email(&self, email: &str) -> Result<UserDocument> {
    ...
    pub async fn get_by_session(&self, session_id: impl Into<bson::Uuid>) -> Result<UserDocument> {
    ...
    pub async fn insert(&self, doc: &UserDocument) -> Result<ObjectId> {
        let id = self.insert_one(doc, None).await?.as_object_id().unwrap();

        Ok(id)
    }

    pub async fn update(&self, id: ObjectId, update: Document) -> Result<UserDocument> {
        let filter = doc! { "_id": id };
